...
class MainViewModel : ViewModel() {
    ...
    fun onEvent(event: MainViewEvent) {
        when (event) {
            is MainViewEvent.LoginEvent -> login(
                event.userName,
                event.password,
                event.fcmToken,
                event.deviceId
    ...
    private fun verifyToken(context: Context, token: String) {
        viewModelScope.launch {
            ...
            MainRepository(instance).verifyToken(context, token)
        }
    }
    ...
    private fun login(userName: String, password: String, fcmToken: String, deviceId: String) {
        viewModelScope.launch {
            val instance =
                ZoomIntegrationRetro.getRetroInstance().create(ZoomIntegrationApi::class.java)
            when (val result =
                MainRepository(instance).login(userName, password, fcmToken, deviceId)) {
                is Resource.Content -> resultToViewState(
                    Resource.Content(
